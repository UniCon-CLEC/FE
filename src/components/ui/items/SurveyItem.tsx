...
const RecommendIcon = ({ isLiked }: { isLiked?: boolean }) => (
  <svg
    ...
    className={isLiked ? 'text-[#F03131]' : 'text-gray-900'}
  >
...
const CardComponent = ({ topic, onDelete, onLike }: CardComponentProps) => {
  ...
  return (
    <div
      className="bg-white rounded-xl p-6 shadow-md w-80 font-sans flex flex-col relative min-h-[180px] transition-all duration-200 ease-in-out hover:translate-y-[-5px] hover:shadow-lg"
    >
      {isMine && (
        <button
      ...
      <div className="text-sm text-gray-600 mb-4 whitespace-nowrap overflow-hidden text-ellipsis">
        {tags && tags.length > 0 ? tags.join(' · ') : ''}
      </div>
      <div className="text-xl font-bold leading-tight mb-6 flex-grow text-gray-800">
        {title}
      </div>
